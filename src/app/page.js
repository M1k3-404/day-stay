    ...
    <div className="h-screen w-full bg-[#f8f8ff] flex justify-content items-center">
      ...
      <div className="mx-auto w-3/4 md:w-1/2 opacity-100 z-10">
        ...
        <form onSubmit={handleSubmit} className="grid lg:grid-cols-3 gap-4 my-8">
          <Input type="text" placeholder="Name" value={name} onChange={(e) => setName(e.target.value)} className="text-xs lg:text-base rounded-full" required />
          <Input type="email" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} className="text-xs lg:text-base rounded-full" required />
          <Button type="submit" className="rounded-full bg-[#db2879] hover:bg-[#db2879ef] font-medium text-xs lg:text-base" disabled={loading}>
        ...
        <div className="flex gap-3 justify-center md:justify-start">
          <Link href={"https://www.facebook.com/people/DayStaylk/61571572059968/"}>
            <FaFacebook className="text-xl text-[#db2879]"  />
          </Link>
          <Link href={"https://www.instagram.com/daystay.lk/"}>
            <FaInstagram className="text-xl text-[#db2879]" />
          </Link>
          <Link href={"https://www.tiktok.com/@daystay.lk"}>
            <FaTiktok className="text-xl text-[#db2879]" />
          </Link>
        </div>
      </div>
    </div>
